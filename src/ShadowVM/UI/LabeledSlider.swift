...
class LabeledSlider: NSView {
  ...
  convenience init(labels: [String]) {
    ...
    for (i, label) in self.labels.enumerated() {
      constraints.append(
        NSLayoutConstraint(
          item: label, attribute: .centerX, relatedBy: .equal, toItem: slider, attribute: .centerX,
          multiplier: 2 * (CGFloat(i) + offset) / (CGFloat(self.labels.count - 1) + 2 * offset)
            + 0.0001, constant: 0))
  ...
  func applyAccessibilityIdentifiers(baseID: String) {
    setAccessibilityIdentifier(baseID)
    slider.setAccessibilityIdentifier("\(baseID).slider")
    for (index, label) in labels.enumerated() {
      label.setAccessibilityIdentifier("\(baseID).label.\(index)")
    }
  }
}
